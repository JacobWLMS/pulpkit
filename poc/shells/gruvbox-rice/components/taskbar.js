function renderTaskbar(s) {
  ...
  wins.forEach(w => {
    ...
    const icon = w.icon ? '<img src="' + w.icon + '" onerror="this.style.display=\'none\'">' : '';
    const name = w.app_id || 'unknown';
    html += '<div class="' + cls + '" onclick="send({cmd:\'focus_window\',data:' + w.id + '})" title="' + (w.title||'').replace(/"/g,'&quot;') + '">'
