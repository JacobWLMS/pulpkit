...
use pulpkit_layout::{compute_layout, paint_tree, LayoutResult, Node};
use pulpkit_render::{Canvas, TextRenderer};
use pulpkit_wayland::LayerSurface;

use pulpkit_layout::Theme;

/// A Wayland layer surface managed by the runtime, with its associated
/// widget tree, cached layout, and dirty flag for render coalescing.
...
impl ManagedSurface {
    /// Recompute layout and paint the widget tree onto the surface buffer.
    ///
    /// Clears the surface with the theme's `base` color, runs flexbox layout,
    /// paints the tree, and commits the buffer to the compositor.
    ...
    /// Mark this surface as needing a re-render on the next frame.
