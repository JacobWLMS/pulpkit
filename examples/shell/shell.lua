...
local function poll_bri()
  local r=rc("brightnessctl -m 2>/dev/null");return r and tonumber(r:match(",(%d+)%%"))or 50
end
local function poll_cpu()
  ...
  local b=tonumber(u)+tonumber(s);local t=b+tonumber(n)+tonumber(i)
  return t>0 and math.floor(b*100/t) or 0
end
...
local function poll_ws()
  local r=rc("niri msg -j workspaces 2>/dev/null");if not r then return {} end
  local l={};for id,idx,foc in r:gmatch('"id":(%d+).-"idx":(%d+).-"is_focused":(.-)[,}]') do
...
function update(state, msg)
  ...
  if t=="niri_event" then
    if msg.data and msg.data:find("^Workspaces changed:") then state.ws=poll_ws() end;return state
  end
  if t=="tick" then
...
-- Bar height: 36px logical (72px physical at 2x)
-- Design grid: 4px base unit
-- Icon size: text-lg (18px) — gives ~36px physical, fits in 36px bar with padding
-- Body text: text-sm (13px) — gives ~26px physical
-- Caption: text-xs (11px) — gives ~22px physical

local BAR_H = 36

local function ibtn(icon, action)
  -- Square hit target: 28x28 with the icon centered
  return button({on_click=action, style="w-7 h-7 rounded-full hover:bg-surface items-center justify-center"},
    text({style="text-lg text-fg"}, icon))
end

local function sep()
  return row({style="w-full h-1 bg-overlay rounded my-1"})
end
...
function view(state)
  ...
  -- ═══════ BAR ═══════
  S[#S+1] = window("bar", {anchor="top", height=BAR_H, exclusive=true, monitor="all"},
    row({style="bg-base w-full h-full px-3 items-center gap-2"},
      ...
      row({style="flex-1 items-center gap-1"},
        each(state.ws, "id", function(ws)
          ...
          return button({on_click=msg("ws_go",tostring(ws.idx)),
            style=a and "w-5 h-5 rounded-full bg-primary items-center justify-center"
                    or "w-5 h-5 rounded-full hover:bg-surface items-center justify-center"},
            text({style=a and "text-xs text-base" or "text-xs text-muted"}, tostring(ws.idx)))
        end)),

      -- ── Center: clock ──
      text({style="text-sm text-fg font-medium"}, state.time),
  ...
  if state.popup=="settings" then
    local function tile(icon, label, active, action)
      return button({on_click=action,
        style=(active and "bg-primary" or "bg-overlay").." rounded-lg p-2 items-center gap-1 flex-1"},
        text({style="text-lg "..(active and "text-base" or "text-fg")}, icon),
        text({style="text-xs "..(active and "text-base" or "text-muted")}, label))
    end

    S[#S+1] = popup("settings", {anchor="top right",width=300,height=340,dismiss_on_outside=true},
      col({style="bg-surface w-full h-full rounded-lg p-3 gap-2"},
        -- Header
  ...
  if state.popup=="bat" then
    S[#S+1] = popup("bat", {anchor="top right",width=240,height=80,dismiss_on_outside=true},
      col({style="bg-surface w-full h-full rounded-lg p-3 gap-2"},
        row({style="items-center gap-2"},
          text({style="text-lg text-fg"}, bati(state.bat,state.bat_st)),
          col({style="gap-0"},
  ...
  if state.popup=="wifi" then
    ...
    for _,net in ipairs(state.wifi_nets) do
      ...
      ch[#ch+1]=button({on_click=net.active and msg("wifi_dis") or msg("wifi_con",net.ssid),
        ...
        text({style="text-xs "..c..(net.active and " font-bold" or "")}, net.ssid..(net.secure and " 󰌾" or "")),
        spacer(),
        net.active and text({style="text-xs text-primary"},I.check) or text({style="text-xs text-muted"},net.signal.."%"))
    end
    S[#S+1]=popup("wifi",{anchor="top right",width=280,height=50+n*24,dismiss_on_outside=true},
      col({style="bg-surface w-full h-full rounded-lg p-3 gap-1"},unpack(ch)))
  end

  -- ═══════ POWER POPUP ═══════
  if state.popup=="power" then
    local function pitem(icon,label,action,color)
    ...
    S[#S+1]=popup("power",{anchor="top right",width=260,height=280,dismiss_on_outside=true},
      col({style="bg-surface w-full h-full rounded-lg p-3 gap-2"},
        col({style="gap-0 items-center py-1"},
        ...
        sep(),
        pitem(I.lock,"Lock Screen",msg("lock")),
        pitem(I.suspend,"Suspend",msg("suspend")),
        pitem(I.reboot,"Restart",msg("reboot")),
        pitem(I.shutdown,"Shut Down",msg("shutdown")),
        sep(),
        pitem(I.logout,"Log Out",msg("logout"),"text-error")))
  end
...
function subscribe(state)
  return {interval(1000,"tick"),stream("niri msg event-stream 2>/dev/null","niri_event"),ipc("ipc")}
end
